...
object NewPersistCassandra {
  ...
  def main(args: Array[String]): Unit = {
   ...
   //  val conf = new SparkConf().setAppName("PersistInCassandra").setMaster("local[*]")
    ...
    val sparkConf = new SparkConf().setMaster("local[*]").setAppName("NewPersistCassandraExample")
    ...
    val newDataStream = data.map { line =>
      {
        ...
        if (matcher.matches()) transformLog.createSeq(matcher)
      }
    }
    ...
    //val cassandraClass: CassandraCreateClass = new CassandraCreateClass(sparkConf, keySpace, tableName)
    ...
    val persistRandom  =  streamingContext.cassandraTable(keySpace,tableName).collect
    persistRandom.foreach { row => println("ip: " + row.getString("ip")) }
 ...
 //   newDataStream.saveToCassandra(keySpace, tableName, SomeColumns("id", "ip", "client", "user"))
